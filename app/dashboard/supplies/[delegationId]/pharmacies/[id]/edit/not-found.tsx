...
export default function NotFound({ delegationId }: { delegationId: string }) {
  return (
    <main className="flex h-full flex-col items-center justify-center gap-2 bg-white mt-7 p-10 dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
      ...
      <Notfound404
        error={404}
        message="No se encontró el insumo solicitado."
        goBack={`/dashboard/supplies/${delegationId}/pharmacies`}
      />
    </main>
  );
}
